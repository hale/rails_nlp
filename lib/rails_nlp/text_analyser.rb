module RailsNlp
  class TextAnalyser

    STOP_WORDS = ['a','able','about','across','after','all','almost','also','am','among','an','and','any','are','as','at','be','because','been','but','by','can','cannot','could','dear','did','do','does','either','else','ever','every','for','from','get','got','had','has','have','he','her','hers','him','his','how','however','i','if','in','into','is','it','its','just','least','let','like','likely','may','me','might','most','must','my','neither','no','nor','not','of','off','often','on','only','or','other','our','own','rather','said','say','says','she','should','since','so','some','than','that','the','their','them','then','there','these','they','this','tis','to','too','twas','us','wants','was','we','were','what','when','where','which','while','who','whom','why','will','with','would','yet','you','your']

    def self.analyse(analysable)
      joined_fields = extract(analysable, RailsNlp.configuration.fields)
      joined_fields.split.each do |word|
        word.downcase!
        next if STOP_WORDS.include?(word)
        kw = Keyword.find_or_create_by(name: word)
        Wordcount.create do |wc|
          wc.analysable_id = analysable.id
          wc.keyword_id = kw.id
          wc.count = 9
        end
      end
    end

      private

      def self.extract(analysable, fields)
        "".tap do |str|
          fields.each do |field|
            field_contents = analysable.send(field)
            str << " " + field_contents if field_contents
          end
        end
      end


  end
end
